...
This repository contains the scripts used to generate analyses and plots for Figures 1–3. The workflows compare whole-genome bisulfite sequencing (WGBS) with Oxford Nanopore Technologies (ONT) methylation data across primed, naive, and trophoblast stem cell (TSC) states, and integrate DNA methylation with transposable-element (TE) annotation, RNA expression, genomic coverage, and evolutionary conservation.
...
## ONT filtering and an important reproducibility note

The intended filtered ONT datasets are derived from **primary alignments with MAPQ >= 10**. Scripts that explicitly use this dataset contain `MAPQ10.primary` in their input paths or filenames. In particular, the Figure 1 Panel B and several Figure 2 workflows explicitly reference MAPQ10-primary ONT inputs.

However, not every committed script currently points to a filename that establishes MAPQ10-primary filtering:

- `Figure-1/Panel-C/Density_Scatter_plot` and `Figure-1/Panel-E/Bar_plot.R` reference legacy ONT methylC paths without `MAPQ10.primary` in their names.
- `Figure-3/Panel-C/Plot_Violin_TE_class_DNA_methylation.R` and `Figure-3/Panel_E/01_make_summary_input.R` reference methylation summaries whose filenames do not identify them as MAPQ10-primary.
- A filename containing `cov10` indicates a CpG coverage threshold of at least 10, which is distinct from a read-alignment MAPQ threshold of 10.

Therefore, users reproducing a strictly MAPQ10-primary analysis should verify or replace those legacy paths before execution. The analysis scripts have intentionally not been modified in this repository audit.
...
The ONT paths in this script do not currently identify MAPQ10-primary files; see the reproducibility note above.
...
The bedGraph filenames establish coverage >= 10 but do not themselves establish MAPQ10-primary alignment filtering.
...
Current-code note: the script uses a `tech` column when writing and plotting but does not create that column. This must be resolved before the committed version can run to completion.
...
Required but not generated by Step 01:

- `outputs/LRonly_x_TE.allchr.wo.tsv`, an 11-column LR-window-by-TE overlap table consumed by Step 02.

The committed workflow does not contain the command that creates this overlap table, so it must already exist before Step 02 is run.
...
2. Install the command-line, Python, and R dependencies listed above.
3. Obtain the required methylC, bedGraph, FASTA, GTF, TE annotation, SQuIRE, Dfam, and precomputed summary inputs.
4. Replace every `/BLUES/eric/...` path with an appropriate local path.
5. Verify that all ONT inputs intended for the filtered analyses were derived from primary alignments with MAPQ >= 10.
6. Run numbered scripts in ascending order within each folder.
7. Run unnumbered scripts independently.
8. Confirm that each expected intermediate exists before running the next numbered step.

## Scope

This README documents the committed scripts and their declared inputs and outputs. It does not modify the analyses, reconstruct missing preprocessing steps, or report scientific results not encoded in the repository.
